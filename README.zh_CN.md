...
1. Field 的 type 一般不需要配置，vite-plugin 已经做了代码的静态分析，自动给每一个 class 和 class-member 注入了 `design:type` 和 `design:fields` 两个 metadata，但要求必须显式对变量进行类型标注（即使设置了默认值）。但如在某些极端场景下未能获得正确的类型，可以手动配置 type 属性。
